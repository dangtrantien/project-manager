...
const ClientList = () => {
  ...
  const fetchData = useCallback(() => {
    ...
    axios
      .get(`${host}/api/admin/client/get-active-list`, {
  ...
  const deleteHandler = (id, multiple) => {
    confirm({
      ...
      okType: 'danger',
      onOk() {
        let url = `${host}/api/admin/client/delete-one/${id}`;
        let data = null;

        if (multiple === 'multiple') {
          url = `${host}/api/admin/client/delete-many`;
          data = id;
        }
        ...
        axios
          ...
          .then((res) => {
            ...
            const updateList = clientList.filter(
              (c) => id.indexOf(c._id) === -1
            );

            setSelectedRows([]);
            setClientList(updateList);
          })
          .catch((error) => console.log(error));
      },
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'name',
      dataIndex: 'name',
      ...useFilterSearch('name', 'Tìm kiếm theo tên khách hàng'),
    },
    {
      title: 'Mô tả',
      key: 'short_desc',
