...
const TechStackList = () => {
  ...
  const fetchData = useCallback(() => {
    ...
    axios
      .get(`${host}/api/admin/tech-stack/get-active-list`)
      .then((res) => {
        const dataList = res.data.data.map((val, i) => {
          return { ...val, key: val._id, index: i + 1 };
        });

        setTechStackList(dataList);
  ...
  const deleteHandler = (id, multiple) => {
    confirm({
      ...
      okType: 'danger',
      onOk() {
        let url = `${host}/api/admin/tech-stack/delete-one/${id}`;
        let data = null;

        if (multiple === 'multiple') {
          url = `${host}/api/admin/tech-stack/delete-many`;
          data = id;
        }
        ...
        axios
          ...
          .then((res) => {
            ...
            const updateList = techStackList.filter(
              (ts) => id.indexOf(ts._id) === -1
            );

            setSelectedRows([]);
            setTechStackList(updateList);
          })
          .catch((error) => console.log(error));
      },
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'name',
      dataIndex: 'name',
      ...useFilterSearch('name', 'Tìm kiếm theo tên tech stack'),
    },
    {
      title: 'Mô tả',
      key: 'short_desc',
